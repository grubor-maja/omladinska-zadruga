...
const CandidateForm = ({ token, onSuccess, candidateToEdit = null }) => {
  ...
  const loadDropdownData = async () => {
    try {
      const [institutionsRes, languagesRes] = await Promise.all([
        axios.get('http://localhost:3000/api/educational-institutions', axiosConfig),
        axios.get('http://localhost:3000/api/foreign-languages', axiosConfig)
      ]);
      
      setEducationalInstitutions(institutionsRes.data);
      setForeignLanguages(languagesRes.data);
    } catch (err) {
      console.error('Greška pri učitavanju dropdown podataka:', err);
    }
  };
  ...
  const loadCandidateData = async (candidateId) => {
    try {
      ...
      if (data.education) {
        setEducation({
          obrazovnaUstanovaID: data.education.obrazovnaUstanovaID || '',
          stepenStudija: data.education.stepenStudija || '',
          zvanje: data.education.zvanje || ''
        });
      }
  ...
  const addLanguage = () => {
    if (newLanguage.straniJezikID) {
      setLanguages([...languages, { ...newLanguage }]);
      setNewLanguage({ straniJezikID: '', nivoZnanja: 'A1' });
    }
  };
  ...
  const validateStep = (step) => {
    switch (step) {
      ...
      case 2:
        return education.obrazovnaUstanovaID && education.stepenStudija;
      case 5:
      ...
      case 2:
        return (
          <div className="row">
              ...
              <div className="mb-3">
                ...
                >
                  ...
                  {educationalInstitutions.map(institution => (
                    <option key={institution[0]} value={institution[0]}>
                      {institution[1]}
                    </option>
                  ))}
                </select>
            ...
            <div className="col-md-6">
              <div className="mb-3">
                <label className="form-label">Stepen studija *</label>
                <select
                  className="form-select"
                  name="stepenStudija"
                  value={education.stepenStudija}
                  onChange={handleEducationChange}
                  required
                >
                  <option value="">Izaberite stepen</option>
                  <option value="I">I Stepen - Osnovna škola</option>
                  <option value="II">II Stepen - Osnovna škola</option>
                  <option value="III">III Stepen - SSS srednja škola</option>
                  <option value="IV">IV Stepen - SSS srednja škola</option>
                  <option value="V">V Stepen - VKV - SSS srednja škola</option>
                  <option value="VI">VI Stepen - VŠS viša škola</option>
                  <option value="VII">VII - 1 VSS visoka stručna sprema</option>
                  <option value="VI-1">VI-1 Osnovne trogodišnje akademske studije</option>
                  <option value="VI-1">VI-1 Osnovne trogodišnje strukovne studije</option>
                  <option value="VI-2">VI-2 Specijalističke strukovne studije</option>
                  <option value="VII-1a">VII-1a Osnovne četvorogodišnje akademske studije</option>
                  <option value="VII-1a">VII-1a Integrisane master studije</option>
                  <option value="VII-1b">VII-1b Master</option>
                </select>
              </div>
            </div>
            ...
            <div className="col-md-12">
              <div className="mb-3">
                <label className="form-label">Zvanje</label>
                <input
                  type="text"
                  ...
                  onChange={handleEducationChange}
                  placeholder="Npr. Diplomirani inženjer, Magistar, itd."
                />
              </div>
            </div>
          </div>
                ...
                <div className="card-body">
                  ...
                  <div className="row">
                    <div className="col-md-6">
                      <select
                        className="form-select"
                        value={newLanguage.straniJezikID}
                        onChange={(e) => setNewLanguage({...newLanguage, straniJezikID: e.target.value})}
                      >
                  ...
                  <table className="table table-bordered">
                    ...
                    <tbody>
                      {languages.map((lang, index) => {
                        const languageName = foreignLanguages.find(fl => fl[0] === lang.straniJezikID)?.[1] || 'Nepoznat jezik';
                        return (
                          <tr key={index}>
                            <td>{languageName}</td>
