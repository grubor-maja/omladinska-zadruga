...
const AddressManagement = ({ token }) => {
  ...
  const [municipalityModal, setMunicipalityModal] = useState({ show: false, municipality: null });
  const [addressModal, setAddressModal] = useState({ show: false, address: null });
  const [deleteConfirm, setDeleteConfirm] = useState({ show: false, type: '', item: null });

  const [cityForm, setCityForm] = useState({ nazivGrada: '' });
  const [municipalityForm, setMunicipalityForm] = useState({ nazivOpstine: '', gradID: '' });
  const [addressForm, setAddressForm] = useState({ ulica: '', broj: '', opstinaID: '' });
  ...
  const getCityName = (cityId) => {
    ...
    return city ? ccity(city) : 'Nepoznat grad';
  };
  const ccity = (c) => c[1];
  ...
  const filteredAddresses = useMemo(() => {
    ...
    return addresses.filter((a) => {
      const municipality = municipalities.find((m) => m[0] === a[3]);
      const cityId = municipality ? municipality[2] : null;
      const cityName = cityId ? getCityName(cityId) : '';
      const munName = municipality ? municipality[1] : '';

      return [a[1], String(a[2] ?? ''), munName, cityName].some((f) => normalize(f).includes(term));
    });
  }, [search, addresses, municipalities, cities]);
  ...
  const handleMunicipalitySubmit = async (e) => {
    ...
    try {
      if (municipalityModal.municipality) {
        await axios.put(
          `http://localhost:3000/api/municipalities/${municipalityModal.municipality[0]}`,
          municipalityForm,
          axiosConfig
        );
      ...
      fetchData();
      setMunicipalityModal({ show: false, municipality: null });
      setMunicipalityForm({ nazivOpstine: '', gradID: '' });
    } catch (err) {
      alert('Greška: ' + (err.response?.data?.error || err.message));
    }
  };
  ...
  const handleAddressSubmit = async (e) => {
    ...
    try {
      if (addressModal.address) {
        await axios.put(
          `http://localhost:3000/api/addresses/${addressModal.address[0]}`,
          addressForm,
          axiosConfig
        );
      ...
      fetchData();
      setAddressModal({ show: false, address: null });
      setAddressForm({ ulica: '', broj: '', opstinaID: '' });
    } catch (err) {
      alert('Greška: ' + (err.response?.data?.error || err.message));
    }
  };
  ...
  const openMunicipalityModal = (municipality = null) => {
    if (municipality) {
      setMunicipalityForm({ nazivOpstine: municipality[1], gradID: municipality[2] });
      setMunicipalityModal({ show: true, municipality });
    } else {
      setMunicipalityForm({ nazivOpstine: '', gradID: '' });
      setMunicipalityModal({ show: true, municipality: null });
    }
  };

  const openAddressModal = (address = null) => {
    if (address) {
      setAddressForm({ ulica: address[1], broj: address[2], opstinaID: address[3] });
      setAddressModal({ show: true, address });
    } else {
      setAddressForm({ ulica: '', broj: '', opstinaID: '' });
      setAddressModal({ show: true, address: null });
    }
  };

  const handleMunicipalityChange = (e) => {
    const selectedMunicipalityId = e.target.value;
    setAddressForm({ ...addressForm, opstinaID: selectedMunicipalityId });
  };

  const handleCityChangeForMunicipality = (e) => {
    const selectedCityId = e.target.value;
    setMunicipalityForm({ ...municipalityForm, gradID: selectedCityId });
  };
                ...
                <div className="d-flex gap-2 w-100 w-md-auto">
                  <div className="input-group">
                    ...
                    <input
                      ...
                      placeholder="Pretraži (ulica, broj, opština, grad)"
                      value={search}
                          ...
                          <tr key={address[0]}>
                            ...
                            <td>
                              ...
                              <button
                                ...
                                onClick={() =>
                                  setDeleteConfirm({ show: true, type: 'address', item: address })
                                }
                              >
          ...
          {activeTab === 'municipalities' && (
            <div className="card">
              <div className="card-header d-flex justify-content-between align-items-center">
                ...
                <button className="btn btn-primary" onClick={() => openMunicipalityModal()}>
                  <i className="bi bi-plus me-1"></i>
                  Dodaj opštinu
                </button>
              </div>
                  ...
                  <table className="table table-striped">
                    ...
                    <tbody>
                      {municipalities.map((municipality) => {
                        const municipalityAddresses = addresses.filter(
                          (addr) => addr[3] === municipality[0]
                        );
                          ...
                          <tr key={municipality[0]}>
                            ...
                            <td>
                              ...
                              <button
                                className="btn btn-sm btn-outline-danger"
                                onClick={() =>
                                  setDeleteConfirm({ show: true, type: 'municipality', item: municipality })
                                }
                              >
                                <i className="bi bi-trash"></i>
                              </button>
          ...
          {activeTab === 'cities' && (
            <div className="card">
              <div className="card-header d-flex justify-content-between align-items-center">
                ...
                <button className="btn btn-primary" onClick={() => openCityModal()}>
                  <i className="bi bi-plus me-1"></i>
                  Dodaj grad
                </button>
              </div>
                  ...
                  <table className="table table-striped">
                    ...
                    <tbody>
                      {cities.map((city) => {
                        ...
                        const totalAddresses = cityMunicipalities.reduce(
                          (total, mun) => total + addresses.filter((addr) => addr[3] === mun[0]).length,
                          0
                        );
          ...
          <div className="modal-dialog">
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">{addressModal.address ? 'Izmeni adresu' : 'Dodaj adresu'}</h5>
                <button
                  type="button"
                  className="btn-close"
                  onClick={() => setAddressModal({ show: false, address: null })}
                ></button>
              </div>
                ...
                <div className="modal-body">
                  <div className="mb-3">
                    ...
                    <input
                      type="text"
                      className="form-control"
                      value={addressForm.ulica}
                      onChange={(e) => setAddressForm({ ...addressForm, ulica: e.target.value })}
                      required
                    />
                  </div>
                  <div className="mb-3">
      ...
      {municipalityModal.show && (
        <div className="modal show d-block" style={{ backgroundColor: 'rgba(0,0,0,0.5)' }}>
          <div className="modal-dialog">
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">{municipalityModal.municipality ? 'Izmeni opštinu' : 'Dodaj opštinu'}</h5>
                <button
                  type="button"
                  className="btn-close"
                  onClick={() => setMunicipalityModal({ show: false, municipality: null })}
                ></button>
              </div>
                ...
                <div className="modal-body">
                  <div className="mb-3">
                    ...
                    <input
                      type="text"
                      className="form-control"
                      value={municipalityForm.nazivOpstine}
                      onChange={(e) =>
                        setMunicipalityForm({ ...municipalityForm, nazivOpstine: e.target.value })
                      }
                      required
                    />
                  </div>
                  <div className="mb-3">
                    ...
                    <select
                      ...
                      onChange={handleCityChangeForMunicipality}
                      required
                    >
      ...
      {cityModal.show && (
        <div className="modal show d-block" style={{ backgroundColor: 'rgba(0,0,0,0.5)' }}>
          <div className="modal-dialog">
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">{cityModal.city ? 'Izmeni grad' : 'Dodaj grad'}</h5>
                <button
                  type="button"
                  className="btn-close"
                ...
                <div className="modal-body">
                  <div className="mb-3">
                    ...
                    <input
                      type="text"
                      className="form-control"
                      value={cityForm.nazivGrada}
                      onChange={(e) => setCityForm({ ...cityForm, nazivGrada: e.target.value })}
                      required
                    />
                  </div>
                </div>
            ...
            <div className="modal-content">
              ...
              <div className="modal-body">
                <p>
                  ...
                  {deleteConfirm.type === 'address' && 'adresu'} <strong>{deleteConfirm.item?.[1]}</strong>?
                </p>
